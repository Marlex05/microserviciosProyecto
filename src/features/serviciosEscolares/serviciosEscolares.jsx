...
import "../../styles/serviciosEscolares.css";

// 👇 importa la API del servicio remoto
import {
  seListarGrupos,
  seObtenerGrupo,
  seRegistrarAlumno,
  seCrearGrupo,
  seInscribirAlumno,
  seDesinscribirAlumno,
  seAsignarProfesor,
} from "../../services/serviciosEscolaresApi";
...
export default function ServiciosEscolares() {
  ...
  useEffect(() => {
    (async () => {
      try { setGrupos(await seListarGrupos()); }
      catch (e) { tell("err", e.message || "No se pudieron cargar los grupos"); }
    })();
  }, []);

  // Cargar detalle del grupo cuando cambie el grupoId
  useEffect(() => {
    if (!grupoId) { setGrupoSel(null); return; }
    (async () => {
      try { setGrupoSel(await seObtenerGrupo(grupoId)); }
  ...
  const crearAlumno = async () => {
    try {
      setLoading(true);
      await seRegistrarAlumno(alumno);
      setAlumno({ matricula:"", nombre:"", carrera:"" });
      tell("ok","Alumno registrado");
    } catch(e){
      tell("err", e.message || "Error al registrar alumno");
  ...
  const crearGrupo = async () => {
    try {
      setLoading(true);
      const r = await seCrearGrupo(grupo);
      setGrupos(prev => [...prev, { id:r.id, ...grupo, alumnos:[], profesorId:null }]);
      setGrupo({ nombre:"", carrera:"" });
      tell("ok", `Grupo creado: ${r.id}`);
    } catch(e){
      tell("err", e.message || "Error al crear grupo");
    } finally { setLoading(false); }
  };
  ...
  const inscribirAlumno = async () => {
    try {
      setLoading(true);
      await seInscribirAlumno(grupoId, matricula);
      setGrupoSel(await seObtenerGrupo(grupoId));
      setMatricula("");
      tell("ok","Alumno inscrito");
    } catch(e){
      tell("err", e.message || "Error al inscribir");
    } finally { setLoading(false); }
  };
  ...
  const desinscribirAlumno = async () => {
    try {
      setLoading(true);
      await seDesinscribirAlumno(grupoId, matricula);
      setGrupoSel(await seObtenerGrupo(grupoId));
      setMatricula("");
      tell("ok","Alumno removido");
    } catch(e){
      tell("err", e.message || "Error al desinscribir");
    } finally { setLoading(false); }
  };
  ...
  const asignarProfesor = async () => {
    try {
      setLoading(true);
      await seAsignarProfesor(grupoId, profesorId);
      setGrupoSel(await seObtenerGrupo(grupoId));
      setProfesorId("");
      tell("ok","Profesor asignado");
    } catch(e){
      tell("err", e.message || "Error al asignar profesor");
    } finally { setLoading(false); }
  };
      ...
      <section className="se-card">
        ...
        <div className="se-row">
          ...
          <button
            ...
            disabled={!grupoId}
            onClick={async ()=>{
              try { setLoading(true); setGrupoSel(await seObtenerGrupo(grupoId)); tell("ok","Grupo actualizado"); }
